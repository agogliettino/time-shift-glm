function [Shift_LLH_Max,p,param,param1,total_LLH_1,ten_LLH_0,mean_LLH_1,mean_LLH_0,ten_LLH_1] = fit_shifted_dataPosHDSpd_OneFit_2(generated_spikes,A_mat,data,post,modelType,num_var,matComps)
%% Load the saved data
...
filter = gaussmf(-4:4,[2 0]); filter = filter/sum(filter); %need the fuzzy logic toolbox for this
dt = post(3)-post(2);

%% first, fit unshifted A matrix to learn parameters and extract LLH

%fit model using unshifted input A matrix to learn the parameters, and
%extract the LLH
[testFit,~,param,~,~,~,~,~] = fit_model(A_mat,dt,generated_spikes,filter,modelType,numFolds);
...
for k = 1:num_var
    ...
    for m = 1:numel(shift_vec)
        ...
        [~,log_llh_test,~,~] = LLH_Comp(param,A_shift,generated_spikes);
    ...
    [testFit,~,param1,~,~,~,~,~] = fit_model(A_shift,dt,generated_spikes,filter,modelType,numFolds);
    ...
    [p{k}] = signrank(ten_LLH_1,ten_LLH_0);
